...
public:
    vistaECPHome(
            char kpaddr = KP_ADDR,
            int receivePin = RX_PIN,
            int transmitPin = TX_PIN,
            int monitorTxPin = MONITOR_PIN
    ...
    std::function<void(uint8_t, const char*)>
    zoneStatusChangeCallback;
    ...
    void onZoneStatusChange(std::function<void (uint8_t zone, const char *msg)> callback) {
    ...
    enum zoneState {
        zopen, zclosed, zbypass, zalarm, zfire, ztrouble
    };
...
private:
    ...
    void setup() override {
        ...
        //retrieve zone status from saved persistent global storage to keep state across reboots
        int zs = id(zoneStates);
        int zb = id(zoneBypass);
        int za = id(zoneAlarms);
        for (int x = 1; x < MAX_ZONES + 1; x++) {  //retrieve from persistent storage
            std::string s = "C";
            zoneState z = zclosed;
            if (zs & 1) {
                zones[x].state = zopen;
    ...
    void set_alarm_state(std::string state, std::string code = "") {
        ...
        // Arm away
        if (state.compare("A") == 0 && !vista.statusFlags.armedStay && !vista.statusFlags.armedAway) {
            if (quickArm) {
                vista.write("#2");
            } else if (code.length() == 4) {
                vista.write(code.c_str());
        ...
        // Arm night
        if (state.compare("N") == 0 && !vista.statusFlags.armedStay && !vista.statusFlags.armedAway) {
            if (quickArm) {
                vista.write("#33");
            } else if (code.length() == 4) {
                vista.write(code.c_str());
        ...
        // Disarm
        if (state.compare("D") == 0 && (vista.statusFlags.armedStay || vista.statusFlags.armedAway)) {
        ...
        // Fire command
        if (state.compare("F") == 0) {
            //todo
            return;
        }

        // Panic command
        if (state.compare("P") == 0) {
            //todo
            return;
        }
    }
    ...
    void update() override {
        if (!firstRun && vista.keybusConnected && millis() - asteriskTime > 30000 && !vista.statusFlags.armedAway &&
            !vista.statusFlags.armedStay && !vista.statusFlags.programMode) {
            asteriskTime = millis();
        ...
        if (vista.keybusConnected && vh) {
            if (firstRun) {
                setExpStates(); //restore expander states from persistent storage
            }
            ...
            */
            ...
            if (vista.newExtCmd) {
                ...
                if (vista.extcmd[0] == 0x98) {
                    ...
                    if (vista.extcmd[2] == 0xf1 && z > 0 && z <= MAX_ZONES) {
                        ...
                        if (z > 0) {
                            ...
                            ESP_LOGD("debug", "Got relay address %d channel %d = %d", vista.extcmd[1], z,
                                     vista.extcmd[4]);
                        }
                    } else if (vista.extcmd[2] == 0x0d) { //relay update z = 1 to 4 - 1sec on / 1 sec off
                        if (z > 0) {
                            // relayStatusChangeCallback(vista.extcmd[1],z,vista.extcmd[4]?true:false);
                            ESP_LOGD("debug", "Got relay address %d channel %d = %d. Cmd 0D. Pulsing 1sec on/ 1sec off",
                        ...
                        for (int x = 8; x > 0; x--) {
                            ...
                            if (!z) continue;
                            zs = faults & 1 ? zopen
                                            : zclosed; //check first bit . lower bit = channel 8. High bit= channel 1
                            //only update status for zones that are not alarmed or bypassed
                            if (zones[z].state != zbypass && zones[z].state != zalarm) {
                                if (zones[z].state != zs) {
                                    if (zs == zopen)
            ...
            if (!(vista.cbuf[0] == 0xf7 || vista.cbuf[0] == 0xf9 || vista.cbuf[0] == 0xf2)) {
                return;
            }
            ...
            if (vista.statusFlags.ready) {
                ...
                for (int x = 1; x < MAX_ZONES + 1; x++) {
                    if (
                        (zones[x].state != zbypass && zones[x].state != zclosed) ||
                        (zones[x].state == zbypass && !vista.statusFlags.bypass)
                    ) {
            ...
            //trouble lights
            /*
            currentLightState.trouble = vista.statusFlags.acLoss;
            */
            ...
            currentLightState.fire = vista.statusFlags.fire;
            if (vista.statusFlags.fire) {
                currentSystemState = striggered;
            }

            currentLightState.alarm = vista.statusFlags.inAlarm;
            ...
            */
            ...
            currentLightState.trouble = currentLightState.ac && !currentLightState.bat;
