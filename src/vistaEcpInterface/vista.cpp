...
void Vista::setExpFault(int zone,bool fault) {
    ...
    expFaultBits = (fault ? expFaultBits | (0x80 >> z) : expFaultBits & ((0x80 >> z) ^ 0xFF));
    //setup bit fields for return response with fault values for each zone
...
void Vista::onExp(char cbuf[]) {
    ...
    if (cbuf[2] & 1) {
        ...
        for (idx = 0; idx < MAX_MODULES; idx++) {
            ...
            if (cbuf[3] == (0x01 << (expansionAddr - 13))) {
    ...
    if (type == 0xF1) {
        ...
        if (cbuf[2] & 1) { //address 14/15
            zoneExpanders[idx].relayState =
                    cbuf[6] & 0x80 ? zoneExpanders[idx].relayState | (cbuf[6] & 0x7f) : zoneExpanders[idx].relayState &
                                                                                        ((cbuf[6] & 0x7f) ^ 0xFF);
            lcbuf[3] = (char) cbuf[6];
        } else {
            zoneExpanders[idx].relayState =
                    cbuf[5] & 0x80 ? zoneExpanders[idx].relayState | (cbuf[5] & 0x7f) : zoneExpanders[idx].relayState &
                                                                                        ((cbuf[5] & 0x7f) ^ 0xFF);
            lcbuf[3] = (char) cbuf[5];
        }
    } else {
...
void Vista::write(const char *receivedKeys) {
    ...
    while (receivedKeys[x] != '\0') {
        if (
            (receivedKeys[x] >= 0x30 && receivedKeys[x] <= 0x39) ||
            receivedKeys[x] == 0x23 ||
            receivedKeys[x] == 0x2a ||
            (receivedKeys[x] >= 0x41 && receivedKeys[x] <= 0x44)
        ) {
...
bool Vista::sendPending() {
    return !(outbufIdx == inbufIdx && retries == 0);
}

bool Vista::charAvail() {
    return outbufIdx != inbufIdx;
}
...
void ICACHE_RAM_ATTR Vista::rxHandleISR() {
    ...
    if (digitalRead(rxPin)) {
        ...
        if (rxState == sPulse) {
            ...
            if (b) {
                vistaSerial->write(b, false); //write byte - no parity bit
            }
            ...
            if (b) {
                vistaSerial->write(b, false);  //only send if needed - no parity bit
            }
...
bool Vista::handle() {
    ...
    if (vistaSerial->available()) {
        ...
        if (x == 0xF7) {
            ...
            if (!validChksum(cbuf, 0, F7_MESSAGE_LENGTH)) {
                return 0;
            }
            newCmd = true;
            rxState = sAckf7;
        ...
        if (x == 0xF9) {
            ...
            if (!validChksum(cbuf, 0, cbuf[2] + 3)) {
                return 0;
            }
            newCmd = true;
            onLrr(cbuf, &gidx);
